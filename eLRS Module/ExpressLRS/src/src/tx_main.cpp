...
// Setup TxUSB
  ...
  if (firmwareOptions.is_airport)
  {
    // AirPort already set up BackpackOrLogStrm as a HardwareSerial(1) on the
    // literal UART0 pins above, and devHandset skips starting the CRSF handset
    // in this mode (see devHandset.cpp), so it is safe to alias TxUSB to that
    // same stream instead of creating a second competing HardwareSerial(1)
    // instance on the same peripheral/pins (which corrupts data).
    TxUSB = BackpackOrLogStrm;
  }
  else if (GPIO_PIN_DEBUG_RX == U0RXD_GPIO_NUM && GPIO_PIN_DEBUG_TX == U0TXD_GPIO_NUM)
  {
