...
// Max MAVLink bytes in one CRSF_FRAMETYPE_ELRS_MAVLINK_RAW frame: packet minus sync, len, type and crc
#define CRSF_MAVLINK_RAW_MAX_PAYLOAD (CRSF_MAX_PACKET_LEN - 4)
...
typedef enum : uint8_t
{
    ...
    // ExpressLRS private frame: raw MAVLink bytes tunnelled over the handset UART (both directions).
    // Non-extended layout: [sync][len][0xAA][mavlink bytes, max CRSF_MAVLINK_RAW_MAX_PAYLOAD][crc]
    CRSF_FRAMETYPE_ELRS_MAVLINK_RAW = 0xAA,
} crsf_frame_type_e;
